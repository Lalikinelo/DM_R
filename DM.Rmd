...
library(stringr)
library(purrr)
APE_Type=read.csv("E:/Geonum/M2/R/DM_R-main/data/APE_Type.csv")
data42=read.csv("E:/Geonum/M2/R/DM_R-main/data/geo_siret_42.csv")
```
# 3.1 Mise en place
...
get_clean_data  <-function(num_departement){
  ...
  if (!file.exists(paste("E:/Geonum/M2/R/DM_R-main/data/geo_siret_",num_departement,".csv",sep = "", collapse = NULL))){
    message("No data available, try another departement number")
    } 
  else{    
    data_departement=read.csv(paste("E:/Geonum/M2/R/DM_R-main/data/geo_siret_",num_departement,".csv",sep = "", collapse = NULL))
...
if(!file.exists("E:/Geonum/M2/R/DM_R-main/data/alimRA_entr.csv"))
  {
  write.csv(alimRA_entr,"E:/Geonum/M2/R/DM_R-main/data/alimRA_entr.csv", row.names = FALSE)
  }else
  {
  alimRA_entr=APE_Type=read.csv("E:/Geonum/M2/R/DM_R-main/data/alimRA_entr.csv")
  }
  if(file.exists("E:/Geonum/M2/R/DM_R-main/data/alimRA_entr.csv"))
  {
...
#tuto: https://r-spatial.org/r/2018/10/25/ggplot2-sf-2.html
...
points$decenieCreationEtablissement = toString(floor(year(ymd(points$dateCreationEtablissement))/10)*10)

dpt=st_read(paste0("E:/Geonum/M2/R/DM_R-main/data/","dpt.shp")) %>%  st_set_crs(2154)
dpt = st_transform(dpt, st_crs(4326))
...
tm_shape(points)+
  tm_facets(by="decenieCreationEtablissement")+
  tm_dots() + 
  tm_layout(main.title="Ouverture de boulangerie dans les departements 01,38,42,43,69", main.title.color="slateblue")+
  tm_scale_bar()+
  tm_compass()
```
...
communes=st_read(paste0("E:/Geonum/M2/R/DM_R-main/data/","com.shp")) %>%  st_set_crs(2154)
...
final=summarise(group_by(st_join(communes,commerces),insee_com),n_commerce=n())

tm_shape(final)+tm_polygons(col="n_commerce",style = "log10_pretty",title="Nombre de commerce par commune") +
